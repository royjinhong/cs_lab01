#######################################################################################################
# program :  satLotto_main.py
# Author:    Roy Liu
# Date:      01 February 2024
#
#This program extracts from the saturday lotto archive site and scrape the draw info and the winning
...
fileSaved = savedir + "sat_lotto" + ".txt"
anafile = savedir + "sat_lotto" + "_analysis.txt"

drawncount = 0

# first extract from controlling page the years available in archive

initialURL = permURL + "/saturday-lotto/past-results"
...
for yrURL in archives:
    ...
    for x in soup:
        ...
        for z in x.find_all('a', title=True):
            if not drawnumExtracted:
               dkey = z['title'].split(' ')[3]
